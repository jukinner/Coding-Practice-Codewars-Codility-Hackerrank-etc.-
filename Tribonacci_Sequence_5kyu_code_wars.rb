...
def tribonacci(signature,n)
  ...
  bucket = signature.dup
  while bucket.count < n
  	first, second, third = bucket[-3], bucket[-2], bucket[-1]
  	bucket << (first + second + third)
  end
  bucket
end
...
# Test.assert_equals(v,[1,1,1,3,5,9,17,31,57,105])
