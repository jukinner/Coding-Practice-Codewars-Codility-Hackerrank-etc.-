...
function sortLetters(arr) {
  ...
  flattened.forEach(function(element) {
    if (/[AEIOU]/.test(element.toString().toUpperCase())) {
      vowels.push(element.toString().toUpperCase());
    } else if (/[A-Z]/.test(element.toString().toUpperCase())) {
      consonants.push(element.toString().toUpperCase());  
    }
  });
 answer.push(vowels);
 answer.push(consonants);
return answer
}